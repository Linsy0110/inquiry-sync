...
def fetch_new_customers():
    ...
    updated_time = json.dumps({"start": last_sync_time, "end": now}, separators=(',', ':'))
    ...
    headers = {"external-sign": sign, "Content-Type": "application/json"}

    try:
        response = requests.get(
            "https://api.salesmartly.com/api/v1/chat-user/get-page-list",
            params=params,
            headers=headers,
            timeout=10
        )
        data = response.json()
        print(f"[{datetime.now()}] SaleSmartly响应: {json.dumps(data, ensure_ascii=False)[:300]}")
